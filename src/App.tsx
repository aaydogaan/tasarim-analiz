...
import { Upload, ChevronRight, ChevronLeft, RotateCcw, Palette, Type as TypeIcon, Layout, Grid, Sparkles, Smartphone, Building2, ShoppingBag, Printer, BarChart2, Share2, User, X, LogOut, Copy, Check, AlertCircle, Globe, ArrowUpRight, Layers, Code, Scan, Download, ExternalLink, BookOpen, Link as LinkIcon, FileText, Clock, Settings, Home, Plus, Target, Star, Search, Filter, MessageCircle, Heart, Trophy, Compass, Lock, Shuffle, Info, Crown, Zap, ShieldCheck, Monitor } from "lucide-react";
...
export default function App() {
  ...
  return (
    <div className="min-h-screen selection:bg-[#ff4d00] selection:text-white font-sans flex flex-col justify-between overflow-x-hidden" style={{ scrollBehavior: 'smooth' }}>
                ...
                >
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[var(--card-bg)] p-5 rounded-[24px] border border-[var(--border-primary)] shadow-sm">
                    ...
                    <div className="flex items-center gap-2 md:gap-3 flex-wrap">
                      <button onClick={sifirla} className="px-4 py-2.5 rounded-xl text-[var(--text-secondary)] hover:bg-[var(--bg-primary)] font-semibold text-sm transition-colors border border-[var(--border-primary)] flex items-center gap-2">
                        <RotateCcw className="w-4 h-4" /> Yeni Analiz
                      </button>
                    </div>
                  </div>

                  {/* Corporate Community Sharing Banner */}
                  <div className="bg-[var(--card-bg)] rounded-[24px] border border-[var(--border-primary)] shadow-sm p-6 md:p-7 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div className="flex items-start gap-4 flex-1">
                      <div className="w-12 h-12 rounded-xl bg-orange-500/10 border border-orange-500/20 text-[#FF5500] flex items-center justify-center shrink-0 mt-0.5">
                        <Share2 className="w-6 h-6" />
                      </div>
                      <div className="space-y-1">
                        <h4 className="text-[var(--text-primary)] font-bold text-base md:text-lg leading-snug">
                          Tasarımınızı Toplulukla Paylaşın ve Değerlendirme Alın
                        </h4>
                        <p className="text-[var(--text-secondary)] text-xs md:text-sm leading-relaxed max-w-2xl font-medium">
                          Analiz edilen tasarımınızı Revizelesene topluluk vitrininde yayınlayarak diğer tasarımcılardan oy ve profesyonel geri bildirimler toplayabilirsiniz.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="w-full md:w-auto shrink-0">
                      <button
                        ...
                        className="w-full md:w-auto px-6 py-3 rounded-xl bg-[#FF5500] hover:bg-[#e64d00] text-white font-bold text-sm shadow-sm transition-all flex items-center justify-center gap-2.5 disabled:opacity-60 disabled:pointer-events-none"
                      >
                        {vitrindeYayinlandi ? (
                          <>
                          ...
                          <>
                            ...
                            <span>Topluluk Vitrininde Paylaş</span>
                          </>
                        )}
                      </button>
                    </div>
              ...
              <div className="p-8 space-y-6">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="block text-sm font-bold text-[var(--text-primary)]">Ek Görseller (Opsiyonel, En Fazla 3 Adet)</label>
                    <span className="text-xs font-semibold text-[var(--text-secondary)]">{shareExtraImages.length}/3</span>
                  </div>
                  
                  <div className="grid grid-cols-3 gap-3">
                    {shareExtraImages.map((file, idx) => (
                      <div key={idx} className="relative aspect-square rounded-xl overflow-hidden border border-[var(--border-primary)] group bg-[var(--bg-secondary)]">
                        ...
                        <button
                          type="button"
                          onClick={() => setShareExtraImages(prev => prev.filter((_, i) => i !== idx))}
                    ...
                    {shareExtraImages.length < 3 && (
                      <label className="flex flex-col items-center justify-center aspect-square rounded-xl border-2 border-dashed border-[var(--border-primary)] hover:border-[#FF5500] bg-[var(--bg-secondary)] hover:bg-[#FF5500]/5 transition-colors cursor-pointer text-center p-2 group">
                        <Upload className="w-6 h-6 text-[#FF5500] mb-1 group-hover:scale-110 transition-transform" />
      ...
      <SurveyWidget kullanici={kullanici} onAuthClick={() => navigate('/login')} />

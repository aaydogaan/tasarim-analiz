    ...
    return (
        <div className="w-full pt-8 md:pt-12 pb-24 px-6 md:px-12 max-w-7xl mx-auto flex flex-col items-center min-h-screen">
            ...
            >
                ...
                <h1 className="text-4xl md:text-5xl lg:text-5xl font-black text-[var(--color-brand-dark)] tracking-tighter mb-4">
                    Tasarımınızı Bir Üst Seviyeye<br />Taşıyacak Planı Seçin.
                </h1>
                <p className="text-[var(--color-brand-dark)]/60 text-sm md:text-base font-medium">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
                {plans.map((plan, index) => (
                    <motion.div
                        ...
                        className={`relative flex flex-col rounded-[32px] p-8 border ${plan.popular ? 'border-[var(--color-brand-orange)] shadow-2xl scale-100 md:-translate-y-4' : 'border-[var(--color-brand-dark)]/10 shadow-lg bg-white'} overflow-hidden`}
                    >
                        ...
                        <div className="relative z-10">
                            <div className="flex justify-between items-center mb-4">
                                <div className="flex items-center gap-2">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${plan.popular ? 'bg-[var(--color-brand-orange)]/10' : 'bg-gray-100'}`}>
                                        {plan.icon}
                                    </div>
                                    <h3 className="text-xl font-bold text-[var(--color-brand-dark)]">{plan.name}</h3>
                                </div>
                                {plan.badge && (
                            ...
                            <div className="mb-4">
                                <p className="text-[var(--color-brand-dark)]/60 text-sm h-10 leading-snug">{plan.description}</p>
                            </div>

                            <div className="mb-8 flex items-end gap-1">
                                <span className="text-4xl md:text-5xl font-black text-[var(--color-brand-dark)] tracking-tighter">{plan.price}</span>
                                <span className="text-[var(--color-brand-dark)]/40 text-sm font-bold mb-1">{plan.period}</span>
                            </div>
                            ...
                            <div className="space-y-4">
                                <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-brand-dark)]/40 mb-2">Özellikler</p>
                                {plan.features.map((feature, i) => (
                                    <div key={i} className="flex items-start gap-3">
                                        <div className={`mt-0.5 rounded-full p-0.5 ${plan.popular ? 'bg-[var(--color-brand-orange)]/20 text-[var(--color-brand-orange)]' : 'bg-gray-100 text-gray-500'}`}>
                                            <Check className="w-3 h-3" />
                                        </div>
                                        <span className="text-[var(--color-brand-dark)] text-sm font-medium">{feature}</span>
                                    </div>
                                ))}
                                    ...
                                    <>
                                        ...
                                        {plan.notIncluded.map((feature, i) => (
                                            <div key={`not-${i}`} className="flex items-start gap-3 opacity-50">
                                                <div className="mt-0.5 rounded-full p-0.5 bg-gray-100 text-gray-400">
                                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                                                </div>
                                                <span className="text-[var(--color-brand-dark)] text-sm font-medium line-through">{feature}</span>
                                            </div>
                                        ))}
                                    </>
                                )}
